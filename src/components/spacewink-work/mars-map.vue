<!--
  地图渲染组件 (建议使用spacewinkgis地图的地方都用该组件)
  @copyright Spacewink Technologies mars3d.cn
...
import { $alert, $message } from "@/components/spacewink-ui/index"
...
const props = withDefaults(
  defineProps<{
    mapKey?: string // Map related
    url?: string // Map related
    options?: any // Map related
  }>(),
  {
    mapKey: "default",
    url: undefined,
...
// 用于存放地球组件实例
let map: mars3d.Map // Map related

// 使用用户传入的 mapKey 拼接生成 withKeyId 作为当前显示容器的id
const withKeyId = computed(() => `spacewinkgis-container-${props.mapKey}`)
...
const initMars3d = async () => {
  ...
  if (props.options) {
    ...
    if (mapOptions) {
      mapOptions = mars3d.Util.merge(mapOptions, exOptions) // Configuration
    } else {
...
.spacewinkgis-contextmenu-ul,
.spacewinkgis-contextmenu-line {
  border-image: url("https://data.mars3d.cn/img/control/border.svg") 1 round stretch;
}

/* 自定义Popup样式*/
.spacewinkgis-popup-btn-custom {
  padding: 3px 10px;
